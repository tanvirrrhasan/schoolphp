...
export default function EditClassPageClient({ id }: EditClassProps) {
    ...
    const loadClass = async () => {
      ...
      try {
        ...
        if (data) {
          ...
          setValue("capacity", classDataLoaded.capacity.toString());
          setValue("status", classDataLoaded.status);
          setValue("sections", classDataLoaded.sections?.join(", ") || "");
          if (classDataLoaded.subjects && classDataLoaded.subjects.length > 0) {
            setValue("subjects", classDataLoaded.subjects);
          }
        }
      } catch (error: any) {
        alert(error.message || "ক্লাস লোড করা যায়নি");
      } finally {
        setLoading(false);
      }
    };
  ...
  const onSubmit = async (data: any) => {
    ...
    try {
      const updatedClassData = {
        ...
        subjects: data.subjects.map((subject: any) => ({
          ...
          textbooks: (subject.textbooks || []).map((book: any) => ({
            ...
            name: book.name || book.nameBn || "",
            nameBn: book.name || book.nameBn || "",
            author: book.author || "",
      ...
      await updateDocument("classes", id, updatedClassData);
      router.push("/admin/classes");
    } catch (error: any) {
      alert(error.message || "ক্লাস আপডেট ব্যর্থ হয়েছে");
    } finally {
      setSaving(false);
    }
  };
  ...
  if (!id) {
    return (
      <div className="text-center py-12">
        ...
        <Link href="/admin/classes" className="text-blue-600 mt-4 inline-block">
          ফিরে যান
        </Link>
      </div>
    );
  ...
  if (!classData) {
    return (
      <div className="text-center py-12">
        <p className="text-gray-500">ক্লাস পাওয়া যায়নি</p>
        <Link href="/admin/classes" className="text-blue-600 mt-4 inline-block">
          ফিরে যান
        </Link>
      </div>
  ...
  return (
    <div>
      <div className="mb-8">
        ...
        <h1 className="text-3xl font-bold text-gray-800 mb-2">ক্লাস সম্পাদনা</h1>
      </div>
